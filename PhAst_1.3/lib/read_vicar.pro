...
;---------------read label size----------------
 ...
 label_nbytes=vicgetpars(str, 'LBLSIZE', status=status)
...
;------------get image dimensions-------------
 ...
 if(n_elements(nl) EQ 0) then $
  begin
   nl=fix(vicgetpars(label, 'NL', status=status))
   if(keyword_set(status)) then $
    begin
     if(NOT keyword_set(silent)) then message, status
     return, 0
 ...
 if(n_elements(ns) EQ 0) then $
  begin
   ns=fix(vicgetpars(label, 'NS', status=status))
   if(keyword_set(status)) then $
    begin
     if(NOT keyword_set(silent)) then message, status
     return, 0
 ...
 if(n_elements(nb) EQ 0) then $
  begin
   nb=fix(vicgetpars(label, 'NB', status=status))
   if(keyword_set(status)) then $
    begin
     nb=1
     nb_note=' (by default)'
 ...
 if(n_elements(nlb) EQ 0) then $
  begin
   nlb=fix(vicgetpars(label, 'NLB', status=status))
 ...
 if(n_elements(nbb) EQ 0) then $
  begin
   nbb=fix(vicgetpars(label, 'NBB', status=status))
   if(keyword_set(status)) then $
    begin
     nbb=200
     nbb_note=' (by default)'
...
;-----------------get data format------------------
 ...
 format_note=''
 format=vicgetpars(label, 'FORMAT', status=status)
 if(keyword_set(status)) then $
  begin
