...
function phast_ExtAdjust, ExtMethod, MagType, magVal, magColor,header=header,astr=astr,image_width=image_width,image_height=image_height,pixelscale=pixelscale
  ...
  case ExtMethod of
    ...
    1: begin ; adjust at airmass of observation
       phast_getFieldEpoch, a, d,radius, X0, obsDate,header=header,astr=astr,image_width=image_width,image_height=image_height,pixelscale=pixelscale
...
pro phast_phot_updateFits, image, head, magzero, magzerr, magzbnd, magzclr, magztrm, magznum, imagename=imagename

  ; update FITS header with photometric zeropoint keywords

  common phast_state

  if not keyword_set(imagename) then imagename = state.imagename
  ...
  fits_write, imagename, image, head
end
